# -*- coding: utf-8 -*-
"""two_stage_recommender.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1fEhRnswd7UIzOJKnhO3WlAKB3lrR4ENr
"""

# model.save("drive/MyDrive/Colab Notebooks/GMF_model.h5",d_path)
...
# user_df['timestamp'] = user_df['timestamp'].apply(data_splitter)
...
### As seen from the number of titles and item, they are different than each other. They were supposed to be same.
...
# Deprecated: tf.keras.preprocessing.sequence APIs are not recommended for new code.
# Prefer tf.keras.utils.timeseries_dataset_from_array and the tf.data APIs which provide
# a much more flexible mechanisms for dealing with sequences.
# See the tf.data guide for more details.
...
# user_df_enc_seq['padded_mids'] = (list(padded_mids))
...
# padded_mids_inp_train, padded_mids_inp_test, occ_enc_inp_train,\
#  occ_enc_inp_test,last_items_inp_train,last_items_inp_test, \
#  user_id_enc_train, user_id_enc_test = train_test_split(padded_mids_inp,
#                                                              occ_enc_inp,last_items_inp,
#                                                              user_df_enc_seq['user_id_enc'],
#                                                               train_size = 0.9, test_size = 0.1, random_state = 42)

# print(len(padded_mids_inp))
# print(len(padded_mids_inp_train))

# print(len(occ_enc_inp))
# print(len(occ_enc_inp_train))

# print(len(user_df_enc_seq['user_id_enc']))

len(last_items_inp)

len(occ_enc_inp)

# user_df_enc_seq['user_id_enc'] = user_df_enc_seq['user_id_enc'].astype('int32')
# user_df_enc_seq['padded_mids'] =np.asarray(user_df_enc_seq['padded_mids']).astype(np.int32)
# user_df_enc_seq['occupation_enc'] = user_df_enc_seq['occupation_enc'].astype('int32')
...
for layer in retrieval_model.layers:
...
print('relu',np.array(retrieval_model.get_layer('dense_4').weights)[0].shape)
print('softmax',np.array(retrieval_model.get_layer('final').weights)[0].shape)

print('softmax',out_of_soft.shape)
print('relu',out_of_relu.shape)
