...
class Interactions(object):
    ...
    def tocoo(self):
        ...
        print('ROW',self.num_users)
        print('COL',self.num_items)
    ...
    def to_sequence(self, sequence_length=5, target_length=1):
        ...
        for i, (uid,
            ...
            if uid != _uid:
                ...
                test_users[uid] = uid
                _uid = uid
                # print(i,uid,test_sequences)
                t_seq.append(test_sequences)
                t_users.append(uid)
        ...
        return t_seq,t_users,self.user_map, self.item_map
...
train_t_seq,train_t_users,u_map,i_map = train.to_sequence(L, T)

train_sequences_np = train.sequences.sequences
train_targets_np = train.sequences.targets
train_users_np = train.sequences.user_ids.reshape(-1, 1)

test = Interactions(test_root,
                        user_map=train.user_map,
                        item_map=train.item_map)

test_t_seq,test_t_users,user_map, item_map = test.to_sequence(L, T)
# n_train = sequences_np.shape[0]


test_sequences_np = test.sequences.sequences
test_targets_np = test.sequences.targets
test_users_np = test.sequences.user_ids.reshape(-1, 1)
# test_t_seq, test_t_users = test.to_sequence(L,T)

# for j,i in enumerate(train.tocsr().toarray()[0]):
#     if int(i) == 1:
#         print(j)

# train_sequences_np[0:40]
...
list(i_map.keys()).index("44")

import collections
od = collections.OrderedDict(sorted(item_map.items()))
od
